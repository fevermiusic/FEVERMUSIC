...
async function captureNotaAsPdf() {
  ...
  // ── Logos (si están disponibles en el DOM) embebidos como PNG real ──
  // Puede haber varios logos en el header (Musical Fever + empresas asociadas),
  // se dibujan uno al lado del otro alineados por la base.
  const logoImgs = Array.from(document.querySelectorAll('#verModal .print-header img'));
  const targetH = 34; // logos más chicos en el header del PDF
  const logoGap = 6;
  const logos = [];
  for (const imgEl of logoImgs) {
    if (!imgEl.src || !imgEl.src.startsWith('data:image/png')) continue;
    try {
      const base64 = imgEl.src.split(',')[1];
      const bytes  = Uint8Array.from(atob(base64), c => c.charCodeAt(0));
      const embedded = await pdfDoc.embedPng(bytes);
      const scale = targetH / embedded.height;
      logos.push({ image: embedded, width: embedded.width * scale, height: targetH });
    } catch (e) { /* si falla, simplemente se omite ese logo */ }
  }

  // ── Header: logo(s) a la izquierda, datos de la nota a la derecha ──
  if (logos.length) {
    let logoX = margin - 6;
    for (const l of logos) {
      page.drawImage(l.image, { x: logoX, y: y - targetH, width: l.width, height: l.height });
      logoX += l.width + logoGap;
    }
    page.drawText('Sistema de Gestion de Pedidos', {
      x: margin, y: y - targetH - 14, size: 8.5, font, color: cGrayLt
    });
  } else {
  ...
  y -= 60; // ajustado para el logo más chico (targetH 34)
